...
const doNotReply: { [index: string]: any } = JSON.parse(
  readFileSync("data/do_not_reply.json", "utf-8")
);
...
const shouldReply = async (msg: Message) => {
  if (
    msg.room() &&
    doNotReply["roomNames"]!.includes(await msg.room()!.topic())
  )
    return false;
  if (!msg.room() && doNotReply["userNames"]!.includes(msg.talker().name()))
    return false;
  return true;
};
