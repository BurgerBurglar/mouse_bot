...
import {
  say,
  getMessageText,
  getMessageTextWithoutMentionsTags,
  removeKeyword,
} from "./utils";
import { doNotReply } from ".";
import { weatherForecast } from "./weather";

const keywordMapper: { [index: string]: string | string[] } = JSON.parse(
  readFileSync("data/keywords.json", "utf-8")
);

const quoteNames: string[] = Object.keys(
  JSON.parse(readFileSync("data/quotes.json", "utf-8"))
);
...
const getQuote = async (msg: Message) => {
  ...
  for (let name of quoteNames) {
    if (text.includes(name)) {
      ...
      try {
        const quote = (
          await axios.get("http://localhost:8000/quotes", {
            params: { name, query },
          })
        ).data;
        say(msg, quote);
...
const getGameCountDown = async (msg: Message) => {
  ...
  try {
    const result = (
      await axios.get<Dictionary<string>>(
        encodeURI(`http://localhost:8000/games/${gameName}`)
      )
    ).data;
    if (!result) {
    ...
    for (let gameName of Object.keys(result)) {
      ...
      if (releaseDate) {
        const days: number = getDaysUntil(
          new Date(`${releaseDate}T00:00+08:00`)
        );
...
const getContentsFromFile = async (filename: string) => {
  const lines = await promises
    .readFile(filename, "utf-8")
    .then((res) => res.split("\n"))
    .then((res) =>
      res.filter((line: string) => line.trim() && !line.includes("#"))
    );
  return lines.slice(1);
};
const getContentsFromFolder = async (
  folderName: string,
  suffix: string = "md"
) => {
  ...
  const contents: string[][] = await Promise.all(
    filenames.map(getContentsFromFile)
  );
...
const getIdiomSolitare = async (msg: Message) => {
  ...
  try {
    const output = (
      await axios.get(encodeURI(`http://localhost:8000/idioms/${text}`))
    ).data.next;
...
const getFootballFixtures = async (msg: Message) => {
  ...
  try {
    const output = (
      await axios.get("http://localhost:8000/leagues/", {
        params: { league, include_odds },
      })
    ).data;
    say(msg, `${output}\n#足球机器人`);
    return true;
