...
import re
import pandas as pd
from .constants import idiom_df, clean_complie
...
def is_idiom_solitaire(before_word, after_word):
    ...
    if (
        b_ser["end_word"] == a_ser["head_word"]
        or b_ser["end_pinyin"] == a_ser["head_pinyin"]
    ):
        return True
    return False
...
def next_idioms_solitaire(
    word, count=1, heteronym=True, smaller=False, four_chars_only=True
):
    """
    :param word: str, 成语
...
def auto_idioms_solitaire(word="", max_count=10, heteronym=True):
    ...
    wdf = idiom_df[
        idiom_df["word"].str.contains(r"^{}".format(word), regex=True)
    ]  # 查詢此成語是否存在。如果不存在則直接返回空
    if wdf.shape[0] == 0:
        return result_list
...
def search_idiom(word, position=0, count=1, is_detail=False):
    ...
    """
    if position > 0:
        check_complie = r"^.{{{position}}}{word}.*".format(
            word=word, position=position - 1
        )
